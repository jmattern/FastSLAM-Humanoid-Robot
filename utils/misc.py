...
# x               generic input
# Output
# y               numerically stable soft max output
def soft_max(x):
    x = x - np.max(x)
    out = np.exp(x)
...
# INPUT 
# roll, pitch, yaw  rotations of the desired transformation matrix 
# x, y, z           positions of the transformation matrix
#
# OUTPUT 
# T                 transformation matrix
def get_T(roll,pitch,yaw,x,y,z):
...
def map_correlation(self, im, x_im, y_im, vp, xs, ys):
    #  plt.figure()
...
# Bresenham's ray tracing algorithm in 2D.# FUNCTION: map_correlation
# INPUT 
# sx,sy             start point of ray
# ex,ey             end point of ray
# OUTPUT 
# v                 list of all ray indices between start and end ray points
...
def test_map_correlation():
    ...
    c = map_correlation(MAP['map'],x_im,y_im,Y[0:3,:],x_range,y_range)
